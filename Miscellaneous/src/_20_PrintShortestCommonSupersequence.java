...
public class _20_PrintShortestCommonSupersequence {
    ...
    public static void printLongestCommonSubsequence(String s1, int n1, String s2, int n2) {
        ...
        // for visualizing print the dc matrix
        printMatrix(dc, n1+1, n2+1);
        ...
        while (i != 0) {  // j has become 0 but i still remaining
            str += s1.charAt(i - 1);
            i--;
        }
        while (j != 0) {  // i has become 0 but j still remaining
            str += s2.charAt(j - 1);
            j--;
        }
    ...
    private static void printMatrix(int[][] dc, int rows, int cols) {
